...
export function Button({ className, loading, disabled, type, testId, onClick, children, asChild }: ButtonProps) {
  ...
  return (
    <Comp disabled={loading || disabled} data-testId={testId} onClick={!asChild ? onClick : undefined} className={clsx(`${className || ''} flex items-center w-full text-center justify-center`, {
      ...
      "outline-none border border-solid font-semibold text-white bg-blue-600 p-4 rounded-md flex items-center gap-3 text-sm focus:ring-2 ring-white hover:bg-blue-800 transition-colors": type == 'primary',
    })}>
      {loading ? (
